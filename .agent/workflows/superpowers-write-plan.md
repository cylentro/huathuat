...
2) plan filename should follow this format:[topic]-plan
   2.a) iteration start from 1 and incremental
   2.b) topic is depend on the plan that is created
3) Run:

```bash
python .agent/skills/superpowers-workflow/scripts/write_artifact.py --path artifacts/superpowers/[topic]-plan.md
